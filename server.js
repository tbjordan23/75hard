...
const QUOTES = [
  ...
  { text: "Sometimes you've got to go through hell to reach heaven.", author: "David Goggins" },
  { text: "When you think you are done, you are only 40 percent into what your body's capable of doing.", author: "David Goggins" },
  { text: "Every morning you have two choices: continue to sleep with your dreams, or wake up and chase them.", author: "David Goggins" },
  { text: "It's so easy to be great these days, because everyone else is so weak. If you have any kind of willpower, discipline, you can dominate.", author: "David Goggins" },
  ...
  { text: "Yeah buddy, light weight!", author: "Ronnie Coleman" },
  { text: "Strength does not come from winning. Your struggles develop your strengths.", author: "Arnold Schwarzenegger" },
  { text: "The mind is the limit. As long as the mind can envision something, you can do it.", author: "Arnold Schwarzenegger" },
  { text: "For me, life is continuously being hungry. The meaning of life is not simply to exist, but to move ahead, to go up, to achieve, to conquer.", author: "Arnold Schwarzenegger" },
  { text: "Nothing is impossible.", author: "Arnold Schwarzenegger" },
  ...
  { text: "There is no such thing as shortcuts, only the way of hard work.", author: "Arnold Schwarzenegger" },
  { text: "If you want to be a champion, you can't have any doubts about it.", author: "Arnold Schwarzenegger" },
  { text: "No pain, no gain.", author: "Arnold Schwarzenegger" },
  { text: "Work like hell. I mean you just have to put in the hours.", author: "Arnold Schwarzenegger" },
  { text: "Trust yourself. Break some rules. Don't be afraid to fail. Ignore the naysayers. Work like hell. Give something back.", author: "Arnold Schwarzenegger" },
...
// Follow-up to migrateDayBoundary(): that migration moved the JSON day
// record (including its `photo` field) from PRE_FIX_BUGGY_DATE to
// PRE_FIX_CORRECTED_DATE, but the actual photo *file* on disk is named by a
// hash of owner key + date (see photoFilePath()) — renaming the JSON key
// alone leaves the file sitting under the old date's hash, so it 404s once
// the day record points at the new date. Runs once, guarded by
// data.dayBoundaryPhotosMigrated, independently of whether the day/startDate
// shift happened via migrateDayBoundary or the account already had a
// correctly-dated record (harmless no-op either way — it only acts when the
// corrected path is missing a file, but the buggy-date one has it).
function migrateDayBoundaryPhotos(data) {
  if (data.dayBoundaryPhotosMigrated) return false;
  for (const [key, user] of Object.entries(data.users || {})) {
    const rec = user.days && user.days[PRE_FIX_CORRECTED_DATE];
    if (!rec || !rec.photo) continue;
    const correctedPath = photoFilePath(key, PRE_FIX_CORRECTED_DATE, rec.photo.ext);
    if (fs.existsSync(correctedPath)) continue; // already in the right place
    const buggyPath = photoFilePath(key, PRE_FIX_BUGGY_DATE, rec.photo.ext);
    if (fs.existsSync(buggyPath)) fs.renameSync(buggyPath, correctedPath);
  }
  data.dayBoundaryPhotosMigrated = true;
  return true;
}
