...
- **Icons**: `icons/icon-192.png`, `icons/icon-512.png`, `icons/apple-touch-icon.png` are generated with Pillow — same violet gradient rounded-square badge as the login mark, now with a bold white flame silhouette (originally a "75" wordmark, replaced per the user's request for something with more intensity; a real person's photo was considered and declined — using someone's likeness as product branding without rights is a real exposure, not just a style choice). No in-repo generation script; see git history if you need to regenerate or tweak them.
