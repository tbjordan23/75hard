...
A shared **75 Hard challenge tracker** for a small group (Travis + friends/family), with a soft-penalty scoring rule, per-person nutrition goals (broken out by meal), progress-photo history, a friends system for photo-based accountability, and daily motivational quotes.
...
- **Take photo vs. choose from library**: the Today card has two separate hidden `<input type="file">` elements (`photo-camera-input` has `capture="environment"`, `photo-library-input` doesn't) behind two explicit buttons, both funneled through the shared `handlePhotoFile()` uploader.
- **My photo gallery**: a "My photos" grid at the bottom of the Your Progress card (`renderPhotoGallery()`) lists every date in `me.days` that has a `.photo`, newest first, each linking to the full-size `/api/photo` URL. This is the user's own full history, distinct from a friend's capped last-5-photos strip in the Friends card.
